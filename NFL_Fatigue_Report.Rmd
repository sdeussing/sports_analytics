...
```{r}
totals <- as.data.frame(matrix(NA, nrow = 3, ncol = 3))

for(i in 1:3){
  temp <- sum_pen[which(sum_pen$cluster == i), ]
  
  totals[i, 1] <- sum(sum_pen$total_tackle_pen, na.rm = TRUE)
  totals[i, 2] <- sum(sum_pen$total_formation_pen, na.rm = TRUE)
  totals[i, 3] <- sum(nrow(temp))
}

colnames(totals) <- c("total_tackle_pen", "total_formation_pen", "n_players")
```
