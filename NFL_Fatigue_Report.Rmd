...
for (id in ran_nflIds) {
  ...
  load_plot <- ggplot(player_plot, aes(x = playId, y = maxFI)) +
    geom_point() + 
    geom_smooth(method = "loess", color = "blue") + 
    labs(x = "Play ID", y = "Max FI", title = paste0("Player Fatigue Index in Game - NFL ID: ", id)) +
    theme_minimal()
...
grouped$max_fatigueIndex[is.infinite(grouped$max_fatigueIndex)] <- 0
...
```{r per play data}
test <- grouped %>%
  select(gameId, playId, nflId, load, displayName, max_acc, tackle_pen, formation_pen, max_fatigueIndex,  avg_fatigueIndex) %>%
  distinct() %>%
  arrange(gameId, nflId, playId)
...
```{r select cols}
clustering_columns <- c("max_acc", "avg_fatigueIndex", "max_fatigueIndex", "cum_load", "height", "weight", "age", "load")
```
...
res <- as.data.frame(matrix(NA, nrow = length(players), ncol = 11))

for(i in 1:length(players)){
  temp <- test[which(test$displayName == players[i]), ]
  
  res[i, 1:8] <- colMeans(temp[, clustering_columns], na.rm = TRUE) #average of all columns above
  res[i, 9] <- max(temp$max_acc, na.rm = TRUE) # max of max acceleration
  res[i, 10] <- max(temp$cum_load, na.rm = TRUE) # max of cumulative load
  res[i, 11] <- max(temp$avg_fatigueIndex, na.rm = TRUE) # max of average fatigue index
  res[i,12] <- max(temp$max_fatigueIndex, na.rm = TRUE) #max of max fatigue index
}

names(res) <- c("avg_max_acc", "avg_fatigueIndex", "max_fatigueIndex","avg_cum_load", "height", "weight", "age", "avg_load", "max_acc_max", "max_cum_load_in_game", "max_avg_fatigueIndex", "max_max_fatigueIndex")
...
positive_defense <- data %>% group_by(nflId) %>%
  select(gameId, playId, nflId, forcedFumbleAsDefense, passDefensed, quarterbackHit, soloTackle, tackleAssist, tackleForALoss) %>% summarise(across(-c(gameId, playId, nflId), sum, na.rm = TRUE))
```
