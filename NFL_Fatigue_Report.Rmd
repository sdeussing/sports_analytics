...
res <- as.data.frame(matrix(NA, nrow = length(players), ncol = 12))

for(i in 1:length(players)){
  temp <- test[which(test$displayName == players[i]), ]
  
  res[i, 1:7] <- colMeans(temp[, clustering_columns], na.rm = TRUE)
  res[i, 8] <- max(temp$max_acc, na.rm = TRUE)
  res[i, 9] <- sum(temp$tackle_pen, na.rm = TRUE)
  res[i, 10] <- sum(temp$formation_pen, na.rm = TRUE)
  res[i, 11] <- max(temp$cum_load, na.rm = TRUE)
  res[i, 12] <- max(temp$avg_fatigueIndex, na.rm = TRUE)
}

names(res) <- c("avg_max_acc", "avg_fatigueIndex", "avg_cum_load", "height", "weight", "age", "avg_load", "max_acc_max", "total_tackle_pen", "total_formation_pen", 
                "max_cum_load_in_game", "max_avg_fatigueIndex")
...
Here, we see that the optimal number of clusters is 9.
...
fit9 <- kmeans(x = sdata,
                ...
                nstart = 25, 
                iter.max = 100)
clusters9 <- fit9$cluster
centers9 <- fit9$centers
summary(as.factor(clusters9))
```
Let's move forward with four clusters.
```{r}
cluster <- c(1:9)
center_df <- data.frame(cluster, centers9)
...
- cluster membership then total penalties
- if there is still a bad column, drop bench players --> n. plays involved in
