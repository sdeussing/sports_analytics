...
We hypothesized that total load would decrease later in the game because a player would be more fatigues. For most of the linemen, we see a slight decline throughout the game. 
...
Our dataframe for clustering with include one row per player in our dataset and include 6 metrics.
```{r make df}
...
We have some negative values in our fatigue resistance index, so we will log this value separately before scaling.
```{r scale}
...
In looking at our heatmap, clusters 2 and 4 are of interest. Players in cluster 2 have comparatively high values for load and acceleration, however, these individuals are not becoming fatigued throughout a game. On the other hand, players in cluster 4 are much less fatigue resistant even though they have lower loads and acceleration values.
...
From this clustering analysis, we see some patterns in terms of penalties. Let's again focus on clusters 2 and 4.

- Cluster 2 were the players with higher loads and acceleration values that were fatigue resistant. Cluster 2 has the highest value for formation penalties in a game. Because these players are so explosive in their acceleration, they might be moving too early and getting called for defensive offsides.
- On the other hand, cluster 4 has lower load and acceleration but greater fatigue. This cluster was the second-highest in tackling penalties. Because these players are getting tired later in the game, they may be making less "clean" tackles and getting called for holding or other tackling penalties.
...
- Cluster 2 is significantly best when looking holistically at defensive metrics and resistance of fatigue. Cluster 1 and 4 are both more geared towards supportive plays, but if we want to consider both production and fatigue resistance, cluster 4 is holistically better.
...
In our clustering analysis, we highlighted clusters 2 and 4. Individuals in cluster 2 are lighter, have a higher acceleration, and are key in impact players. These players, however, have the highest number of formation penalties. Although they are explosive, these players are often starting play (exploding) too early. 
Individuals in cluster 4 are heavier and key in assisted plays, however, these individuals suffer from fatigue throughout a game. For this reason, we would recommend endurance training for these individuals. 

After training or at the start/end of a season, any coach or team can rerun our clustering analysis to see if individual players have shifted between clusters. This would indicate that players have become more/less endurant, and then training can be adjusted accordingly.

In future work, we would like to apply a similar fatigue analysis to other player groups, such as quarterbacks, runningbacks, and offensive linemen.
