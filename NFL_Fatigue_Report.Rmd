...
weeks <- data.frame(read.csv("weeksAll.csv"))
dim(weeks)
...
```
```{r}
plot2 <- inner_join(summary_def, names_penalties) %>% mutate(cluster = as.factor(cluster))

saveRDS(plot2, "playerplot2.RData")
g_2 <- ggplot(plot2, # Set dataset
              aes(x = soloTackle_AvgPerGame,  # Set x-axis as number of fastballs
  ...
  geom_text_repel(data = plot2,
   ...
   theme(panel.grid.major = element_blank(), # Remove grid
        ...
        panel.border = element_blank(), # Remove grid
        panel.background = element_blank()) + # Remove grid
  labs(x = "Avg Solo Tackles Per Game", y = "Avg Total Penalties per Game",
