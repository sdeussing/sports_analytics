...
Let's dive further into these acceleration values. Within one play, we want to look at the change in acceleration and load. We will split the data by position to see any potential positional differences.
...
- change in max acceleration over a game --> per play level (per player)
- cumulative load over a game (add load each play) --> per play level (per player) 
  - don't restrict frame
